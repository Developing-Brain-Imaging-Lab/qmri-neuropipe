...
- Replaced the legacy `dmipy>=1.0` integration with the supported
  `dmipy-fit>=2.3,<2.4` analytical fitting backend.
...
- Added the Garcia-Hernandez et al. microglia/astrocyte activation model using
  dmipy's finite-pulse S4 Gaussian-phase sphere implementation.
...
- Explicitly disabled dmipy-fit 2.3 NEXI JAX fitting because its released JAX
