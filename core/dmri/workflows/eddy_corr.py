...
def perform_eddy(dwi_image, working_dir, topup_base=None, method='eddy', gpu=False, cuda_device=0, nthreads=1, fsl_eddy_options=None, tortoise_options=None, struct_img=None, verbose=False):
    ...
    if not eddycorrected_img.exists():
        ...
        if( eddycorrected_img.slspec == None ):
            dmri_qc.create_slspec_file(input_dwi     = dwi_image,
                                       output_base   = output_base)
