...
def prep_anat_rawdata(bids_id, bids_rawdata_dir, bids_derivative_dir, bids_t1w_dir='anat', bids_t2w_dir='anat', t1w_reorient_img=None, t2w_reorient_img=None, t1w_type='t1w', nthreads=1, verbose=False):
    ...
    if t1w_type == 'mp2rage':
        ...
        raw_t1w = Image(file = bids_t1w_rawdata_dir + bids_id + '_rec-MoCo_MPnRAGE.nii.gz',
        ...
        if coreg_t2.exists():
            ...
            if t1w_reorient_img is not None and t2w_reorient_img is not None:
                ...
                reg_tools.linear_reg(input_img      = raw_t1w,
                                     ...
                                     method         = 'FSL',
                                     dof            = 6,
                                     flirt_options =  '-cost normcorr -searchcost normcorr -interp sinc -searchrx -180 180 -searchry -180 180 -searchrz -180 180 -init ' + bids_t2w_derivative_dir + bids_id + '_space-reorientImg_T2w.mat')
