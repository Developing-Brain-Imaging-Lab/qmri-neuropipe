...
#include "qmri/models/despot2_fm.hpp"
...
Usage:
  qmri_fit_despot1 --ssfp=<file> --params=<json> --t1=<file> --b1=<file> --out_dir=<dir>
                   [--algo=<a>] [--out_base=<file>] [--mask=<file>][--f0=<file>] [--lambda=<l>] [--mu=<m>] [--reg=<r>] 
...
Options:
  ...
  --t1=<file>        T1 map NIfTI (float) [default: ].
  --b1=<file>        External B1 map NIfTI (float) [default: ].
  --f0=<file>        B0-inhomogeneity map NIfTI [default: ].
  --mask=<file>      Mask NIfTI; fit where mask==1.
  --out_base=<file>  Output basename [default: DESPOT1_]
  --algo=<a>         CERES | SRC | NLOPT | OLS | WLS [default: CERES].
...
int main(int argc, char** argv){
    ...
    Eigen::MatrixXf F = Eigen::MatrixXf::Zero(2, V);
    ...
    qmri::DESPOT2FMParams params(Eigen::Map<const Eigen::VectorXf>(flips_deg.data(), Nobs),
                                 Eigen::Map<const Eigen::VectorXf>(TRs.data(), Nobs),
                                 Eigen::Map<const Eigen::VectorXf>(phases.data(), Nobs));
    
    qmri::DESPOT2FMBinder despot2(params, F);
    ...
    if(verbose){
        ...
        std::cout << indent << indent << "T1 Map: " << std::filesystem::absolute(t1_path) << std::endl;
...
//      
  ...
  for (int outer=0; outer<outer_iters; ++outer) {
    ...
    output_paths[2] = (out_dir/(out_base+"F0.nii.gz")).string();
    ...
    if(verbose){
        ...
        std::cout << indent << indent << "F0 Map: " << std::filesystem::absolute(output_paths[2]) << std::endl;
        std::cout << std::endl;
